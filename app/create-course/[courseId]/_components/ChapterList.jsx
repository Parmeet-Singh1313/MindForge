                ...
                {course?.courseOutput?.course?.chapters?.map((chapter, index) => (
                    <div className="border p-5 rounded-lg mb-2 flex items-center justify-between">
                        <div className="flex gap-5 items-center">
                            <h2 className="bg-primary h-10 w-10 text-white rounded-full text-center p-2 flex-none">{index + 1}</h2>
                            <div>
                        ...
                        {edit ? <HiOutlineCheckCircle className="text-4xl text-gray-300 flex-none" /> : <HiOutlineCheckCircle className="text-4xl text-green-600 flex-none" />}
                    </div>
                ))}
            </div>
        </div>
